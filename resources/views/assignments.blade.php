...
<div class="space-y-4" id="eventList">
    ...
    @forelse ($futureEvents as $event)
        ...
        <div
            ...
            onclick='openActivities({{ $event["id"] }}, @json($event["name"]))'
        >
            <div>
    ...
    @foreach ($doneEvents as $event)
        ...
        <div
            ...
            data-event-id="{{ $event['id'] }}"
            style="display: none;"
            onclick='openActivities({{ $event["id"] }}, @json($event["name"]), "done")'
        >
            <div>
                <p class="font-bold text-lg text-[#0e243a] mb-1">
                    {{ strtoupper($event['name']) }}
...
<script>
    const eventDates = @json($events->pluck('date')->filter()->values());
</script>
<script src="{{ asset('js/assignment-management.js') }}"></script>
